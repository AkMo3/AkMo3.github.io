...
public final class ScopeUtil {
    ...
    public static Scope getSurroundingScope(DetailAST node) {
        ...
        final boolean isNodeModifierPresent = nodeModifier != null;
        boolean isScopePubicForInterface = isNodeModifierPresent
                && getScopeFromMods(nodeModifier) == Scope.PUBLIC;
        if (isPublicFieldInInterfaceBlock(node, isScopePubicForInterface)
                && isInterfacePublic(node)) {
            returnValue = Scope.PUBLIC;
        }
        else {
            boolean isInsideTypeDeclaration = false;
            for (DetailAST token = node.getParent();
                 token != null;
                 token = token.getParent()) {
                final int type = token.getType();
                // This condition is valid either when node is nested within 2 type declaration
                // or until this condition the scope is public
                if (TokenUtil.isOfType(token, TokenTypes.INTERFACE_DEF)) {
                    returnValue = getSurroundingScopeInsideInterfaceDef(token, returnValue);
                }
                else if (TokenUtil.isTypeDeclaration(type)) {
                    isInsideTypeDeclaration = true;
    ...
    private static Scope getSurroundingScopeInsideInterfaceDef(DetailAST token, Scope returnValue) {
        final DetailAST mods =
                token.findFirstToken(TokenTypes.MODIFIERS);
        final Scope modScope = getScopeFromMods(mods);
        if ((returnValue == null || returnValue.isIn(modScope)
    ...
    private static boolean isInterfacePublic(DetailAST node) {
        ...
        for (DetailAST token = node.getParent();
            ...
            if (token.getType() == TokenTypes.INTERFACE_DEF) {
                final DetailAST nodeModifier = token.findFirstToken(TokenTypes.MODIFIERS);
                if (nodeModifier != null && getScopeFromMods(nodeModifier) == Scope.PUBLIC) {
                    returnValue = true;
                }
            }
